...
export default function BrandClient() {
    ...
    useEffect(() => {
            const getBrandDetails = async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error("Brand Details Fetch Failed !");
                    }
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                // Parallel fetching for recent viewed + category children
    ...
    }, [slug, filterBrand, searchParams.toString(), page]);
                ...
                <div className="container">
                    <div className="row">
                                        ...
                                        {categories.map(cat => (
                                            <li className={`${(cat.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === cat.id)) ? 'active' : ''}`} key={cat.id}>
                                                <div className="list-text">
                                                    <Link href={`/product-category/${cat.slug}`} className={``} >
                                                        {cat.name}
                                                        ...
                                                        {cat.children_recursive.map((child) => (
                                                            (child.products_count > 0 &&
                                                                <li className={`${(child.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === child.id)) ? 'active' : ''}`} key={child.id}>
                                                                    <div className="list-text">
                                                                        <Link href={`/product-category/${cat.slug}/${child.slug}`} className={''}>
                                                                            {child.name}
                                                                            ...
                                                                            {child.children_recursive.map((value) => (
                                                                                (value.products_count ?
                                                                                    <li key={value.id} className={`${(value.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === value.id)) ? 'active' : ''}`}>
                                                                                        <div className="list-text">
                                                                                            <Link href={`/product-category/${cat.slug}/${child.slug}/${value.slug}`} className={""}>
                                                                                                {value.name}
                                                                                            ...
                                                                                            <ul className="sub-list">
                                                                                                {value.children_recursive.map((final) => (
                                                                                                    (final.products_count ?
                                                                                                        <Link href={`/product-category/${cat.slug}/${child.slug}/${value.slug}/${final.slug}`} key={final.id} >
                                                                                                            {final.name}
                        ...
                        <div className="col-md-7">
                                                    ...
                                                    <div className="row">
                                                        <div className="col-md-3">
                                                            <div className="img-container">
                                                                <img src={brandDetail?.image_url} alt="" />
                                                            </div>
                                                        </div>
                                                        <div className="col-md-7">
                            ...
                            <div className="widget">
                                <h4><Link href="/">Home</Link> / <Link href={`/brand/${brand}`}>{brand}</Link>
